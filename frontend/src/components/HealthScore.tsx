...
export const HealthScore: React.FC<HealthScoreProps> = ({
  ...
  const effectiveBand = isInsufficient ? 'INSUFFICIENT HISTORY' : (band ? band.replace('_', ' ') : 'HEALTHY');
  ...
  if (size === 'sm') {
    return (
      <span className="inline-flex items-center gap-1.5 font-mono text-data">
        ...
        <span className={`px-1.5 py-0.5 text-[10px] uppercase border ${badgeColor}`} style={{ borderRadius: '2px' }}>
          {effectiveBand}
        </span>
      </span>
    );
  ...
  return (
    <div className="p-4 border bg-surface-1 border-border" style={{ borderRadius: '2px' }}>
      <div className="flex items-center justify-between">
        ...
        <span className={`px-2 py-0.5 text-[10px] font-mono font-medium uppercase border ${badgeColor}`} style={{ borderRadius: '2px' }}>
          {effectiveBand}
        </span>
      </div>
      ...
      <div className="flex items-baseline gap-3 mt-2">
        ...
        <div className={`flex items-center gap-1 text-data font-medium ${trendColor}`}>
          ...
          <span className="text-[11px] font-mono uppercase">{isInsufficient ? 'INSUFFICIENT HISTORY' : (trend ? trend.replace('_', ' ') : 'STABLE')}</span>
        </div>
      </div>

      {isInsufficient ? (
        <p className="mt-3 text-[11px] text-slate-400 leading-normal border-t border-border-subtle pt-2 font-mono">
          <strong className="text-slate-300">Baseline in Progress:</strong> Minimum 12 observations required in evaluation window to calculate statistically calibrated health index.
        </p>
      ) : showDisclaimer ? (
